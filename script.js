...
taskInput.addEventListener("keypress", function (e) {
  //listen for keypress on the input element, and run the following function
  if (e.key === "Enter") {
...
function saveTasks(tasks) {
  //define function saveTasks which takes tasks as an argument
...
function addTask() {
  ...
  if (tasks.some((t) => t.text.toLowerCase() === text.toLowerCase())) {
    //if the entered text is the same as some text that already exists,
    alert("That task already exists!"); //show alert warning
    return; //end the addTask function
  }

  const newTask = {
    //define object newTask
...
function renderTask(task) {
  //define the renderTask function
